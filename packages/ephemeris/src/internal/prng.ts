...
/**
 * A reproducible seed for the permutation nulls. State it in
...
function bytesFromHex(hex: string, what: string): Uint8Array {
  if (hex.length === 0 || hex.length % 2 !== 0 || !/^[0-9a-fA-F]+$/.test(hex)) {
    throw new EphemerisError(
      'invalid_options',
      `${what}: hex seed must be non-empty even-length hex`,
    )
  }
 ...
 * `[01 00]` distinct.
 *
 * @throws {EphemerisError} `invalid_options` for a negative, fractional, oversized, or
 *   malformed seed.
 */
export function seedToUint64(seed: Seed, what = 'seed'): bigint {
  if (typeof seed === 'number') {
    if (!Number.isSafeInteger(seed) || seed < 0) {
      throw new EphemerisError(
        'invalid_options',
        `${what} must be a non-negative safe integer, got ${seed}`,
      )
    }
    return BigInt(seed)
  }
  if (typeof seed === 'bigint') {
    if (seed < 0n || seed > MASK64) {
      throw new EphemerisError(
        'invalid_options',
        `${what} bigint must be in [0, 2^64), got ${seed}`,
      )
    }
    return seed
  }
...
/**
 * The package's internal deterministic PRNG (the same generator as @mindpeeker/psi, so a seed means the same stream in both): xoshiro128** (Blackman & Vigna 2018,
...
export class Xoshiro128 {
  ...
  #uniform(n: number): number {
    if (n === 1) return 0
    // x mod n as x − ⌊x/n⌋·n: exact for x, n ≤ 2^32 (the double quotient never rounds across an
    // integer) and several times faster than the engine's floating-point `%` in this hot loop.
    const limit = Math.floor(TWO_32 / n) * n
    for (;;) {
      const x = this.nextUint32()
      if (x < limit) return x - Math.floor(x / n) * n
    }
  }
   ...
   * likely.
   */
  shuffle<A extends { length: number; [index: number]: number }>(items: A): A {
    if (items.length > TWO_32) {
      throw new EphemerisError(
        'invalid_options',
        `cannot shuffle more than 2^32 items, got ${items.length}`,
      )
    }
