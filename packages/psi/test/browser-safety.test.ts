 ...
 * dependency `@mindpeeker/negentropy` (itself browser-safe) and its
 * `/numerics` subpath.
 */
...
describe('browser safety', () => {
  ...
  test('src/ has no node: builtins and only the negentropy bare import', () => {
      ...
      for (const match of content.matchAll(importPattern)) {
        ...
        expect(
          specifier.startsWith('./') ||
            specifier.startsWith('../') ||
            specifier === '@mindpeeker/negentropy' ||
            specifier === '@mindpeeker/negentropy/numerics',
