/**
 * Synchronous SHA-256 (FIPS 180-4) for the few places that need a digest
 * without awaiting WebCrypto — e.g. a provider name fingerprint that must be
 * known at construction. Bulk hashing stays on `crypto.subtle`.
 */
...
export function sha256(message: Uint8Array): Uint8Array {
  ...
  for (let offset = 0; offset < padded.length; offset += 64) {
    ...
    for (let t = 0; t < 64; t++) {
      const sigma = ((e >>> 6) | (e << 26)) ^ ((e >>> 11) | (e << 21)) ^ ((e >>> 25) | (e << 7))
      const t1 = (hh + sigma + ((e & f) ^ (~e & g)) + (K[t] as number) + (w[t] as number)) >>> 0
      const rho = ((a >>> 2) | (a << 30)) ^ ((a >>> 13) | (a << 19)) ^ ((a >>> 22) | (a << 10))
      const t2 = (rho + ((a & b) ^ (a & c) ^ (b & c))) >>> 0
      hh = g
      g = f
      f = e
      e = (d + t1) >>> 0
