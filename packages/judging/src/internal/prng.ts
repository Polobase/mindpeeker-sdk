...
/**
 * A reproducible seed for judging's Monte Carlo permutation PRNG. State it in
...
function bytesFromHex(hex: string, what: string): Uint8Array {
  if (hex.length === 0 || hex.length % 2 !== 0 || !/^[0-9a-fA-F]+$/.test(hex)) {
    throw new JudgingError('invalid_options', `${what}: hex seed must be non-empty even-length hex`)
  }
 ...
 * `[01 00]` distinct.
 *
 * @throws {JudgingError} `invalid_options` for a negative, fractional, oversized, or
 *   malformed seed.
 */
export function seedToUint64(seed: Seed, what = 'seed'): bigint {
  ...
  if (typeof seed === 'bigint') {
    if (seed < 0n || seed > MASK64) {
      throw new JudgingError('invalid_options', `${what} bigint must be in [0, 2^64), got ${seed}`)
    }
    return seed
  }
  const bytes =
...
/**
 * judging's internal deterministic PRNG (identical to psi's): xoshiro128** (Blackman & Vigna 2018,
 ...
 *
 * Statistical, not cryptographic: it decides *which* permutations
